...
public interface DoctorRepository extends JpaRepository<Doctor, Integer> {
    ...
    @Query(
            value = "SELECT DISTINCT doctor.id, doctor.personal_data_pesel FROM doctor\n"
                    ...
                    + "WHERE specialization.name LIKE ?1 AND clinic.city LIKE ?2",
            nativeQuery = true)
    ...
    @Query(
            value = "SELECT DISTINCT doctor.id, doctor.personal_data_pesel FROM doctor\n"
                    + "join doctor_specializations on doctor.id = doctor_specializations.doctor_id\n"
                    + "join specialization on specialization.id = doctor_specializations.specializations_id\n"
                    + "WHERE specialization.name LIKE :specialization",
            nativeQuery = true)
